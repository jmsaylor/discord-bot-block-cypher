...
const transactMicro = async (from_private, to_address, value_satoshis) => {
  let microtx = {
  ...
  // microtx = JSON.stringify(microtx)

  let transaction = await fetch(
    `https://api.blockcypher.com/v1/ltc/main/txs/micro?token=${tokenBlockCypher}`,
    {
      method: "POST",
      body: microtx,
    }
